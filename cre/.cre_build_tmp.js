...
var USDC = 1000000n;
var MAX_MULTIPLE = 3;
var DEFAULT_NO_HISTORY_BPS = 4000;
var MIN_CREDIT_ALLOCATION_BPS = 1000;
var MAX_CREDIT_ALLOCATION_BPS = 7000;
var REPORT_TTL_SECONDS = 7n * 24n * 60n * 60n;
var USDC_DECIMALS = 1e6;
function deriveConservativeCap(input, inference) {
  const sanitizedMultiple = Math.max(0, Math.min(MAX_MULTIPLE, inference.approvedMultiple));
  const marginAdjustedMrr = input.monthlyRecurringRevenueUsd * Math.max(0, input.grossMarginBps) / 1e4;
  const burnCoveragePenalty = input.monthlyBurnUsd > input.cashBalanceUsd ? 0.5 : 1;
  const delinquencyPenalty = input.delinquencyRateBps > 1000 ? 0.5 : 1;
  const riskPenalty = inference.riskScore > 70 ? 0.5 : 1;
  const capUsd = marginAdjustedMrr * sanitizedMultiple * burnCoveragePenalty * delinquencyPenalty * riskPenalty;
  return BigInt(Math.floor(capUsd)) * USDC;
}
function hasPlatformHistory(input) {
  const history = input.platformTrackRecord;
  if (!history)
    return false;
  return history.repaymentCount > 0;
}
function deriveCap(input, inference) {
...
function deriveCreditAllocationBps(input, inference) {
  if (!hasPlatformHistory(input))
    return DEFAULT_NO_HISTORY_BPS;
  const history = input.platformTrackRecord;
  if (!history)
    return DEFAULT_NO_HISTORY_BPS;
  const onTimeBps = Math.max(0, Math.min(1e4, history.onTimeRepaymentBps));
  const repaymentDepthBps = Math.min(1000, history.repaymentCount * 200);
  const volumeDepthBps = Math.min(1000, Math.floor(history.totalRepaidUsdc / 1000) * 100);
  const onTimeBonusBps = Math.floor((onTimeBps - 8000) / 4);
  const latePenaltyBps = Math.min(2500, history.lateRepaymentCount * 800);
  ...
  const rawBps = DEFAULT_NO_HISTORY_BPS + onTimeBonusBps + repaymentDepthBps + volumeDepthBps - latePenaltyBps - outstandingPenaltyBps - riskPenaltyBps - delinquencyPenaltyBps - burnPenaltyBps;
  return Math.floor(Math.max(MIN_CREDIT_ALLOCATION_BPS, Math.min(MAX_CREDIT_ALLOCATION_BPS, rawBps)));
}
function deriveSupplyFromPrincipal(input, creditAllocationBps) {
  const principalUnits = BigInt(Math.floor(Math.max(0, input.currentDepositedPrincipalUsdc) * USDC_DECIMALS));
  return principalUnits * BigInt(Math.floor(creditAllocationBps)) / 10000n;
}
function encodeCreditCapReport(vendor, cap, expiry, creditAllocationBps) {
  return encodeAbiParameters2(parseAbiParameters2("address vendor, uint256 cap, uint64 expiry, uint16 creditAllocationBps"), [vendor, cap, expiry, creditAllocationBps]);
}
var platformTrackRecordSchema = exports_external.object({
  drawdownCount: exports_external.number(),
  repaymentCount: exports_external.number(),
  onTimeRepaymentCount: exports_external.number(),
  lateRepaymentCount: exports_external.number(),
  onTimeRepaymentBps: exports_external.number(),
  totalRepaidUsdc: exports_external.number(),
  currentOutstandingDebtUsdc: exports_external.number(),
  currentDebtDueAt: exports_external.number()
});
var financialInputsSchema = exports_external.object({
  vendor: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/),
  currentDepositedPrincipalUsdc: exports_external.number(),
  monthlyRecurringRevenueUsd: exports_external.number(),
  grossMarginBps: exports_external.number(),
  cashBalanceUsd: exports_external.number(),
  monthlyBurnUsd: exports_external.number(),
  delinquencyRateBps: exports_external.number(),
  platformTrackRecord: platformTrackRecordSchema.nullish()
});
var configSchema = exports_external.object({
  inferenceUrl: exports_external.string().min(1),
  evm: exports_external.object({
    chainSelectorName: exports_external.string(),
    receiver: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/),
    gasLimit: exports_external.string()
  })
});
function decodePayload(payload) {
  const inputText = Buffer.from(payload.input).toString("utf8");
  return financialInputsSchema.parse(JSON.parse(inputText));
}
function fetchInference(sendRequester, inferenceUrl, financials) {
...
function onHttpTrigger(runtime2, payload) {
  ...
  runtime2.log(`starting underwriting for ${financials.vendor}`);
  const httpClient = new cre.capabilities.HTTPClient;
  const inference = httpClient.sendRequest(runtime2, fetchInference, ConsensusAggregationByFields({
    riskScore: median,
    approvedMultiple: median,
    rationale: identical
  }))(runtime2.config.inferenceUrl, financials).result();
  runtime2.log(`inference complete with risk=${inference.riskScore}`);
  const nowSeconds = BigInt(Math.floor(runtime2.now().getTime() / 1000));
  ...
  const encodedPayload = encodeCreditCapReport(financials.vendor, cap, expiry, creditAllocationBps);
  ...
  if (!network286) {
    throw new Error(`Unsupported network ${runtime2.config.evm.chainSelectorName}`);
  }
  const report2 = runtime2.report(prepareReportRequest(encodedPayload)).result();
  const evmClient = new cre.capabilities.EVMClient(network286.chainSelector.selector);
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.evm.receiver,
    report: report2,
    gasConfig: { gasLimit: runtime2.config.evm.gasLimit }
  }).result();
  if (writeResult.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`writeReport failed: ${writeResult.errorMessage || writeResult.txStatus}`);
  }
  const txHash = writeResult.txHash ? bytesToHex3(writeResult.txHash) : "0x";
  runtime2.log(JSON.stringify({
